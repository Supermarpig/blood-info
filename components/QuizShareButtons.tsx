...
/** 通用分享列（LINE / Threads / IG＋複製），文字與網址由呼叫端組好傳入。 */
export default function QuizShareButtons({ shareText, shareUrl, label = "分享結果" }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <div className="mb-3 grid grid-cols-3 gap-3">
        <button
          onClick={handleLine}
          className="flex flex-col items-center gap-2 rounded-xl bg-[#00B900]/10 p-3 transition-colors hover:bg-[#00B900]/20"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-2 rounded-xl bg-gray-100 p-3 transition-colors hover:bg-gray-200"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-2 rounded-xl bg-gray-100 p-3 transition-colors hover:bg-gray-200"
        >
          <span className="text-gray-900">{IG_ICON}</span>
          <span className="text-xs font-medium text-gray-900">IG</span>
        </button>
      </div>
      <button
        onClick={handleCopy}
        className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-50"
      >
        {copied ? (
          <>
