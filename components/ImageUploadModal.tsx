...
const fileSchema = z.object({
  imageFile: z
    .instanceof(File)
    .refine((file) => file.size <= 5000000, `文件大小不能超過 5MB.`)
    .refine(
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {image ? (
          <button className="bg-gray-200 rounded-full flex items-center justify-center size-16 overflow-hidden relative">
            <Image
              src={image}
              alt="Uploaded"
              fill
      ...
      <DialogContent
        aria-describedby={image ? "dialog-description" : undefined}
        className="max-w-[425px] w-[90%]"
      >
        <DialogHeader>
        ...
        {image ? (
          <div className="space-y-4">
            {isSubmitted && (
              <p id="dialog-description" className="text-green-500 text-center">
                圖片提交成功，等待審核！
              </p>
            )}
            <Image
          ...
          <Tabs defaultValue="url" className="w-full">
            <p id="dialog-description" className="hidden">
              支援 URL 🔗 或文件上傳📁
            </p>
