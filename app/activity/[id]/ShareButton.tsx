...
const THREADS_ICON = (
  <svg viewBox="0 0 192 192" className="w-6 h-6 fill-current" xmlns="http://www.w3.org/2000/svg">
...
export default function ShareButton({ event, giftNames, pageUrl }: Props) {
  const [copied, setCopied] = useState(false);

  const getShareText = () => {
    const giftLine = giftNames.length > 0 ? `\n贈品：${giftNames.join("、")}` : "";
    return `【捐血活動】${event.organization}\n時間：${event.activityDate} ${event.time}\n地點：${event.location}${giftLine}\n\n詳細資訊：${pageUrl}`;
  };
  ...
  return (
    <div className="mt-4 bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
      <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-3 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        <button
          onClick={handleLine}
          className="flex flex-col items-center gap-2 p-3 rounded-xl bg-[#00B900]/10 hover:bg-[#00B900]/20 transition-colors"
        >
          <span className="text-[#00B900]">{LINE_ICON}</span>
        ...
        >
          <span className="text-gray-900">{THREADS_ICON}</span>
          <span className="text-xs font-medium text-gray-900">Threads</span>
        </button>
        <button
          onClick={handleIG}
          className="flex flex-col items-center gap-2 p-3 rounded-xl bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50 hover:from-purple-100 hover:via-pink-100 hover:to-orange-100 transition-colors"
        >
          <span className="bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 bg-clip-text text-transparent">
            {IG_ICON}
          </span>
          <span className="text-xs font-medium bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent">IG</span>
        </button>
      </div>
      <button
        onClick={handleCopy}
